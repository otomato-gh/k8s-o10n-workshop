...
Putting togeher @@LINK[scripts/hpa-v2-pa-httplat.yaml]:

.small[
```yaml
@@INCLUDE[scripts/hpa-v2-pa-httplat.yaml]
```
]
...
- Create the HorizontalPodAutoscaler:
  ```bash
  kubectl apply -f ~/k8s-o10-workshop/scripts/hpa-v2-pa-httplat.yaml
  ```

- Check the logs of the `controller-manager`:
  ```bash
  kubectl logs --namespace=kube-system --tail=10 -lapp=controller-manager
  ```
